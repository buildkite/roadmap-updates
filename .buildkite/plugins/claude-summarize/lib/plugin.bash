...
function create_annotation() {
  ...
  echo "--- :memo: Creating annotation (scope: ${scope})"
  ...
  # Use job ID for context when job-scoped so each job gets its own annotation
  local context_id="${BUILDKITE_BUILD_ID}"
  local annotate_args=(
    --style "${style}"
  )

  if [ "${scope}" = "job" ]; then
    context_id="${BUILDKITE_JOB_ID:-${BUILDKITE_BUILD_ID}}"
    annotate_args+=(--context "claude-analysis-${context_id}")
  ...
  buildkite-agent annotate "${annotate_args[@]}" < "${annotation_file}"
}
...
function analyze_build_times() {
...

  {
    ...
    echo ""
    echo "$([ "${is_step_level}" = "true" ] && echo "Step" || echo "Build") Time Statistics:"
